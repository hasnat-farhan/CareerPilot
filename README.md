...
> � **Live now →** [https://careerpilot-1-tpbe.onrender.com](https://careerpilot-1-tpbe.onrender.com) — full system design in [`docs/SYSTEM_DESIGN.md`](docs/SYSTEM_DESIGN.md) § *Deployment*.
...
```
              ...
              └────────────┬───────────────────────────────────────────┘
                           │  Clerk session
                           ▼
   ┌───────────── Next.js 15 App Router (Render Web Service) ─────────┐
   │                                                                   │
...
| Layer | Choice | Why |
|---|---|---|
| **Framework** | Next.js 15.5 (App Router, RSC, Turbopack) | Single repo for UI + API, server actions cut latency, Render Web Service deploy |
...
| **Hosting** | Render (Web Service) | Single Node 20 service running `npm run start`; `/api/cv/upload` runs in 60 s (sync parse) |
...
Never commit `.env.local`. Render environment variables are set in the Web Service dashboard.
